...
int ATOS::evaluate(const std::vector<std::vector<char>> &board) const
{
    for (int i = 0; i < 3; i++)
    {
...
bool ATOS::isFull(const std::vector<std::vector<char>> &board) const
{
...
// Minimax algorithm con alpha-beta pruning
int ATOS::minimax(std::vector<std::vector<char>> &board, int depth, bool isMaximizer, int alpha, int beta)
{
...
// Mejor movimiento
Move ATOS::findBestMove(const std::vector<std::vector<char>> &board)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            if (board[i][j] == ' ')
            {
                ...
                int moveValue = minimax(tempBoard, 0, false, std::numeric_limits<int>::min(), std::numeric_limits<int>::max());
